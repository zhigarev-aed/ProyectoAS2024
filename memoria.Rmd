...
**Umbrales** 
Se cambiara el parámetro policy de la función explicada anteriormente: threshold(), el cual determina el valor del umbral utilizado en el proceso de eliminación de ruido. Se exploran tres enfoques diferentes para la selección del umbral: universal, fdr y manual
...
```
 ...
 Una vez tenemos las imágenes con ruido generadas y ampliadas en potencia de 2 en este caso de 2.048x2.048 pixeles, aplicamos la función imwd a cada uno de los tres canales (R, G y B). Empleamos la función procesar_imagen_wavelet que devuelve las imágenes reconstruidas después del thresholding.
 
 Aplicamos en cada ruido dos umbrales diferentes Universal y fdr y un umbral tipo 'hard'. Presentamos las imagenes sin el relleno para que sea más facil de visualizar y ocupamos una función del paquete magick para juntar 3 imagenes y agregar título, esto lo hacemos en la función `imprimir`. 
...
En los siguientes 3 tipos de ruidos salt and pepper, uniforme y gamma funciona bien el umbral fdr, 
...
El umbral Universal utiliza un umbral fijo basado en el tamaño de la señal, lo que no captura completamente la variabilidad del ruido, en cambio FDR estima la probabilidad de que un coeficiente de wavelet provenga del ruido. Como resultado, el FDR podría ofrecer una eliminación de ruido más adaptativa, ayudando a preservar mejor los detalles de la imagen.
