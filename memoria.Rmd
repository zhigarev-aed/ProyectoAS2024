...
resize_imwd<- function(foto){
  ...
  img <- as.cimg(foto) # Pasar a formato Imager
...
```{r}
images_recortadas <- lapply(images_for_imwd_cut, resize_imwd)

```

```{r}
 # Visualizamos las imágnes redimensionadas
...
```{r}
 # Visualizamos las imágenes recortadas sin ruido
par(mfrow = c(1, 3), mar = c(1, 1, 1,1))
